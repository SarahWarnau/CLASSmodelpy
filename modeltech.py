...
class model:
    ...
    def init(self):
        ...
        self.Tsurf      = self.input.theta                  # surface layer temperature [K]
    ...
    def timestep(self):
        self.temp_from_pottemp() # Calculates Tair_s from theta and Tsurf from thetasurf
        self.statistics()
        ...
        if(self.sw_sl):
            self.run_surface_layer() # Calculates thetasurf, recalculates Tsurf, calculates T2m
    ...
    def statistics(self):
        ...
        self.P_h    = self.Ps - self.rho * self.g * self.h # hydrostatic balance
        self.T_h    = self.Tair_s - self.g/self.cp * self.h
        ...
        while(((RHlcl <= 0.9999) or (RHlcl >= 1.0001)) and it<itmax):
            ...
            T_lcl        = self.Tair_s - self.g/self.cp * self.lcl
    ...
    def run_surface_layer(self):
        ...
        self.Tsurf = self.thetasurf * (self.Ps / self.P0)**(self.Rd / self.cp)
        qsatsurf       = qsat(self.Tsurf, self.Ps)
    ...
    def jarvis_stewart(self):
        ...
        f4 = 1./ (1. - 0.0016 * (298.0-self.Tair_s)**2.)
    ...
    def ags(self):
        ...
        CO2comp       = self.CO2comp298[c] * self.rho * pow(self.Q10CO2[c],(0.1 * (self.Tsurf - 298.)))  

        # calculate mesophyll conductance
        gm            = self.gm298[c] *  pow(self.Q10gm[c],(0.1 * (self.Tsurf-298.))) \
                          / ( (1. + np.exp(0.3 * (self.T1gm[c] - self.Tsurf))) * (1. + np.exp(0.3 * (self.Tsurf - self.T2gm[c]))))
        ...
        Ammax         = self.Ammax298[c] *  pow(self.Q10Am[c],(0.1 * (self.Tsurf - 298.))) / ( (1. + np.exp(0.3 * (self.T1Am[c] - self.Tsurf))) * (1. + np.exp(0.3 * (self.Tsurf - self.T2Am[c]))))
    ...
    def run_land_surface(self):
        ...
        # calculate skin temperature implictly
        self.Ts   = (self.Q  + self.rho * self.cp / self.ra * self.Tair_s \
            + self.cveg * (1. - self.cliq) * self.rho * self.Lv / (self.ra + self.rs    ) * (self.dqsatdT * self.Tair_s - self.qsat + self.q) \
            + (1. - self.cveg)             * self.rho * self.Lv / (self.ra + self.rssoil) * (self.dqsatdT * self.Tair_s - self.qsat + self.q) \
            + self.cveg * self.cliq        * self.rho * self.Lv /  self.ra                * (self.dqsatdT * self.Tair_s - self.qsat + self.q) + self.Lambda * self.Tsoil) \
        ...
        self.LEveg  = (1. - self.cliq) * self.cveg * self.rho * self.Lv / (self.ra + self.rs) * (self.dqsatdT * (self.Ts - self.Tair_s) + self.qsat - self.q)
        self.LEliq  = self.cliq * self.cveg * self.rho * self.Lv / self.ra * (self.dqsatdT * (self.Ts - self.Tair_s) + self.qsat - self.q)
        self.LEsoil = (1. - self.cveg) * self.rho * self.Lv / (self.ra + self.rssoil) * (self.dqsatdT * (self.Ts - self.Tair_s) + self.qsat - self.q)
...
class model_output:
    def __init__(self, tsteps):
        ...
        self.Tsurf      = np.zeros(tsteps)    # surface layer temperature [K]
        ...
        self.Tsoil      = np.zeros(tsteps)    # soil temperature [K]
...
class model_input:
    def __init__(self):
        ...
        self.Tsurf      = None  # surface layer temperature [K]
